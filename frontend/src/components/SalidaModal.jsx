...
function SalidaModal({ open, onClose, onSubmit, productos }) {
  ...
  const validateItem = (item) => {
    const itemErrors = {};
    if (!item.producto) itemErrors.producto = 'Debe seleccionar un producto';
    if (!item.cantidad || parseFloat(item.cantidad) <= 0) itemErrors.cantidad = 'La cantidad debe ser mayor a 0 (ej: 5)';
    if (!item.cargo) itemErrors.cargo = 'Debe seleccionar un cargo';
    if (item.cargo === 'maquinaria' && !selectedMaquinaria) itemErrors.cargo = 'Debe seleccionar un equipo de maquinaria';
    return itemErrors;
  };
  ...
  const handleSubmit = () => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      // Verificar stock mínimo antes de enviar
      const productosPorDebajo = salidaItems.filter(item => {
        const prod = item.producto;
        return prod && parseFloat(prod.stock) - parseFloat(item.cantidad) <= parseFloat(prod.stock_minimo);
      });

      if (productosPorDebajo.length > 0) {
        alert("Atención: Uno o más productos quedaron por debajo del stock mínimo. Por favor crear solicitud antes del término del día.");
      }
